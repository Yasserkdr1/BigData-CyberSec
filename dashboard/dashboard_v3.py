...
def build_alerts_query(limit_value: int) -> str:
    return f"""
...
<body>
        ...
        <div id="streaming-view" class="view-section active">
            ...
            <div class="dashboard-grid">
                <div class="card span-3">
                    <div class="kpi-label">Afficher les dernières alertes</div>
                    <select id="live-limit-select" class="live-select" onchange="fetchLiveAlerts()">
                ...
                <div class="card span-3">
                    <div class="kpi-label">Alertes chargées</div>
                    <div class="kpi-value" id="kpi-live-count">0</div>
                </div>
    ...
    <script>
        ...
        async function fetchLiveAlerts() {
            try {
                const limit = document.getElementById('live-limit-select')?.value || 100;
                ...
                document.getElementById('kpi-live-count').textContent = payload.count;
                ...
                if (!alerts.length) {
                    tbody.innerHTML = '<tr><td colspan="7" class="empty">Aucune alerte détectée pour cette vue.</td></tr>';
                } else {
                ...
                document.getElementById('last-refresh-live').textContent =
        ...
        async function fetchBatchStats() {
            try {
                ...
                document.getElementById('last-refresh-batch').textContent =
                    'MAJ: ' + new Date().toLocaleTimeString('fr-FR');
            } catch (e) {
...
def get_alerts():
    try:
        ...
        query = build_alerts_query(limit_value)
